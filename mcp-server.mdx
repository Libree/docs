...
Pond3r provides a [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) server that lets AI agents query curated crypto datasets directly. The server uses **SSE transport**, so it works with Claude Desktop, Cursor, and any MCP-compatible client.

## Getting Your API Key

<Steps>
  <Step title="Create an account">
    Sign up at [pond3r.xyz](https://pond3r.xyz) if you haven't already.
  </Step>
...
For any MCP-compatible client that supports SSE transport, connect to:

```
https://mcp.pond3r.xyz/sse
```
...
<AccordionGroup>
  <Accordion title="Connection refused or timeout">
    Make sure your API key is valid and correctly set. Check that your client supports SSE transport and is sending the `Authorization` header.
  </Accordion>
